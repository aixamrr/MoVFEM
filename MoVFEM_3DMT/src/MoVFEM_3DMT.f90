...
program MoVFEM_3DMT
	...
	!USE IFPORT !Only when using IFORT compiler, if using gnu compiler comment this line
	use kind_param
	use solution, only: node_solution,write_solution
	use global_assembly, only:nne,nnze,find_zeros,rem_zeros,sym
	use problem, only:ndir,write_problem_ugrid, pset_pmodel
	...
	use boundary_conds, only: bd_updatemodel
    implicit none
    !Working variables
	...
	!initialize mpi
    	...
    	!------------------------------------------------------------
    	!MUMPS Ordering Scheme, Modify it to use different schemes
		mumps_par%icntl(7)=5 !ordering scheme: 4-ford 5-METIS
		...
		!call estimate_memory() !Uncomment if you want to know the memory usage by the program (for testing purposes)
!---------------------------------------------------------------
!---------Frequency Loop----------------------------------------
	do 	ii=1,nf
	    if (mumps_par%myid.eq.0) then
			...
			total2=total
	 		...
	 		hr=mod(total2,60.)
	 		write(*,*) 'Time running MoVFEM assembly for freq. no: ', ii
		...
		mumps_par%job=6
		mumps_par%icntl(14)=50 !Increase memory allowance by 50%, default is 20%
		call zmumps(mumps_par)
		...
		if (mumps_par%myid.eq.0) then
	    	...
	    	!****write solutions***************
	   	 	write(*,*) 'Nodal solution for E and H'
			...
			total2=total
	 		...
	 		total2=(total2-mins)/60.
 			hr=mod(total2,60.)
 			write(*,*) 'Time running MoVFEM solution for freq. no: ', ii
	 		write(*,'(f12.5,a,a,i3,a,i2,a,i2)') total,'(sec)','=',int(hr),':',int(mins),':',int(sec) 	
...
!----------------------------------------------------------------------------
    subroutine initialize_vfem()
    	...
    	use kind_param
		use read_input, only: in_dx, in_dy, in_dz, in_high, in_depth, in_mn, in_bcksigma, in_nsf,&
		...
		implicit none
		!Working variables
		integer:: outp,air,inimod, in_nextd, symmetry,&
		...
		if (mumps_par%myid.eq.0) then
			write(*,*)'-----------------------------------------------------'
			write(*,*)'Multi-order Vector Finite Element Method (MoVFEM) for'
			write(*,*)'3D Magnetotelluric Forward Modelling'
			write(*,*) 'by Aixa M. Rivera-Rios, Bing Zhou & Graham Heinson'
			write(*,*)'The University of Adelaide, South Australia'
			write(*,*)'-----------------------------------------------------'
			write(*,*) 'Initializing MoVFEM'
			!Read 3DEM_FEM.inp file and initialize input variables
			call input()
			air=0; inner=1; symmetry=1
			...
			print*, 'Background Sigma: ', in_bcksigma
			call grid_3d(in_dx, in_dy, in_dz, in_high, in_depth, in_mn, in_bcksigma, in_nsf,&
	    	in_nsp, in_xto, in_yto, in_zto, in_nsr, in_xsr, in_ysr, in_zsr,in_mx,in_my,&
	...
	!
			...
			!Using Electric Field problem and both polarizations
			call init_problem(1, 2,1)
	    	...
	    	call ga_init(symm,dirich,inimod)
    ...
    subroutine estimate_memory()
        use boundary_conds, only: bdry_mem
        use geometry, only: geo_mem
        use global_assembly, only: ga_mem
        use integration, only: int_mem
        use n_fem, only:nfem_mem
        use problem,only: prob_mem
        use read_input, only:input_mem
        use receiver_data, only:rx_mem
        use solution, only: sol_mem
        use v_fem, only: vfem_mem
        integer:: X=0
        call bdry_mem(X)
        call geo_mem(X)
        call ga_mem(X)
        ...
        call sol_mem(X)
        call vfem_mem(X)
        print*, 'Estimated Memory used in MoVFEM: ', X
